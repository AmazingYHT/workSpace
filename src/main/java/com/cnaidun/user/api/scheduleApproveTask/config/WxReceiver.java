package com.cnaidun.user.api.scheduleApproveTask.config;//package com.cnaidun.user.api.scheduleApproveTask.config;
import java.io.*;
...
//    private Map<String, Object> consumerConfigs;
    ...
    private static final Logger logger = LoggerFactory.getLogger(WxReceiver.class);

    @Async("asyncServiceExecutor")
    public void sendWx() {
        while (true) {
            logger.info("***************开始发送微信信息*******************");
            logger.info("start executeAsync");

            //      for(;;){
            List<WeiXinDataInterChange> list = Collections.synchronizedList(weiXindataInterChangeService.selectUserInfo());
            ...
            if (list.size() == 0) {
                ...
                List<WorkType> workList = Collections.synchronizedList(weiXindataInterChangeService.selectAll());

                List<IdInfo> idList = Collections.synchronizedList(weiXindataInterChangeService.selectIdInfo());
